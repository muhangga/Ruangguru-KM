...
func (p *PlaylistRepository) FetchPlaylistTrack(playlistID int64) ([]model.PlaylistTrack, error) {
	...
	// TODO: answer here
	sqlStatement = `
		SELECT p.id, p.name, t.id, t.name, t.artist FROM playlist_tracks pt JOIN playlists p ON p.id = pt.playlist_id JOIN tracks t ON t.id = pt.track_id WHERE p.id = ?;
	`
	...
	for rows.Next() {
		...
		err := rows.Scan(&playlistTrack.PlaylistID, &playlistTrack.PlaylistName, &playlistTrack.TrackID, &playlistTrack.TrackName, &playlistTrack.TrackArtist)
		if err != nil {
			return nil, err
		}
